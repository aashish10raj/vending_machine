...
const ManageBuyers = () => {
  ...
  useEffect(() => {
    const getUsers = async () => {
      try {
        ...
        await Promise.all(
          usersData.map(async (user) => {
            balanceData[user.user_id] = await fetchUserBalance(user.user_id);
          })
        );
        setBalances(balanceData);
      } catch (error) {
  ...
  const handleUpdateName = async (userId) => {
    ...
    if (newName) {
      try {
        await updateUserName(userId, newName);
        setUsers(users.map(user => user.user_id === userId ? { ...user, Email: newName } : user));
      } catch (error) {
        console.error("Error updating name:", error);
      }
    }
  ...
  return (
    <div className="container mt-4">
      ...
      {showAddForm && (
        <form onSubmit={handleAddUser} className="card p-3 mb-4">
          <input type="email" name="email" value={newUser.email} onChange={handleInputChange} placeholder="Email" required className="form-control mb-2" />
          <input type="password" name="password" value={newUser.password} onChange={handleInputChange} placeholder="Password" required className="form-control mb-2" />
          <label>
            <input type="checkbox" name="isAdmin" checked={newUser.isAdmin} onChange={handleInputChange} /> Is Admin
          </label>
          <button type="submit" className="btn btn-primary">Add User</button>
        </form>
      )}
      ...
      <div className="row">
        {(showAll ? users : users.slice(0, 6)).map((user) => (
          <div key={user.user_id} className="col-md-4 mb-4">
            <div className="card h-100">
              <div className="card-body">
                <h5 className="card-title">{user.Email}</h5>
                <p className="card-text">User ID: {user.user_id}</p>
                <p className="card-text">Role: {user.IsAdmin ? "Admin" : "Buyer"}</p>
                <p className="card-text">Balance: ₹{balances[user.user_id] || 0}</p>
                <button className="btn btn-sm btn-warning" onClick={() => handleUpdateName(user.user_id)}>Update Name</button>
                <button className="btn btn-sm btn-danger" onClick={() => handleDeleteUser(user.user_id)}>Delete</button>
                <button className="btn btn-sm btn-info mt-2" onClick={() => handleUpdateBalance(user.user_id)}>Update Balance</button>
              </div>
            </div>
          </div>
        ))}
      ...
      {!showAll && users.length > 6 && (
        <button className="btn btn-primary" onClick={() => setShowAll(true)}>Show all users</button>
      )}
      {showAll && (
        <button className="btn btn-info" onClick={() => setShowAll(false)}>Collapse</button>
      )}
    </div>
  );
};
